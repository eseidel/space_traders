...
Future<ShipNav> setShipFlightMode(
  ...
  final response = await api.fleet
      .patchShipNav(ship.symbolString, patchShipNavRequest: request);
...
Future<NavigateShip200ResponseData> navigateShip(
  ...
  final result = await method(ship.symbolString, navigateShipRequest: request);
...
Future<SiphonResources201ResponseData> siphonResources(
  ...
  final response = await api.fleet.siphonResources(ship.symbolString);
  ship
...
Future<ExtractResources201ResponseData> extractResources(
  ...
  final response = await api.fleet.extractResources(ship.symbolString);
  ship
    ..cargo = response!.data.cargo
    ..cooldown = response.data.cooldown;
  await db.upsertShip(ship);
...
Future<ExtractResources201ResponseData> extractResourcesWithSurvey(
  ...
  final response = await api.fleet
      .extractResourcesWithSurvey(ship.symbolString, survey: survey);
  ship
    ..cargo = response!.data.cargo
    ..cooldown = response.data.cooldown;
  await db.upsertShip(ship);
...
Future<SellCargo201ResponseData> sellCargo(
  ...
  final response =
      await api.fleet.sellCargo(ship.symbolString, sellCargoRequest: request);
  final data = response!.data;
  ship.cargo = data.cargo;
  await db.upsertShip(ship);
  await agentCache.updateAgent(Agent.fromOpenApi(data.agent));
...
Future<SellCargo201ResponseData> purchaseCargo(
  ...
  final response = await api.fleet
      .purchaseCargo(ship.symbolString, purchaseCargoRequest: request);
  final data = response!.data;
  ship.cargo = data.cargo;
  await db.upsertShip(ship);
  await agentCache.updateAgent(Agent.fromOpenApi(data.agent));
...
Future<RefuelShip200ResponseData> refuelShip(
  ...
  final responseWrapper =
      await api.fleet.refuelShip(ship.symbolString, refuelShipRequest: request);
